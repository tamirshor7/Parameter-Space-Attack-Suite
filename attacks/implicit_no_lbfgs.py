...
class ArgminLayer(nn.Module):
    def __init__(self, inner_steps, inner_lr, inr, inner_optimizer_fn, voxels):
        super(ArgminLayer, self).__init__()
        self.inner_steps = inner_steps
        self.inner_lr = inner_lr
        self.inr = inr
        self.inner_optimizer_fn = inner_optimizer_fn
        self.voxels = voxels
    ...
    def forward(self, x, return_mse = False, clean = False):
        ...
        if clean:
            ...
            prod = (-hessianyx.T@hessianyy.inverse())
...
class Implicit(nn.Module):
    ...
    def __init__(self, inr, classifier, inner_steps = 100, inner_lr = 0.01, voxels = False, device='cuda'):
        ...
        self.argmin = ArgminLayer(inner_steps, inner_lr, self.inr, self.inner_optimizer_fn, voxels)
...
def run_attack(model, loader, criterion, constraint, pgd_iters, pgd_lr, voxels = False, device='cuda'):
    ...
    for x,labels in prog_bar:
        ...
        clean, clean_mod,dvdx,initial_mse = model(x.unsqueeze(1), return_mse = True, clean=True)
        ...
        for iter in range(pgd_iters):
            ...
            if voxels:
                ...
                mask.mask_grad(lr=args.lr, grad=((dldv@dvdx)*dxdp).reshape(1,15,15,15)) 
        ...
        with torch.no_grad():
            attack_succ = (output.argmax(int(voxels)) != labels).item()
            if not attack_succ:
                rights_attacked += 1
...
def get_args():
    ...
    parser.add_argument('--mod-steps', type=int, default=5, help='Number of internal modulation optimization steps per PGD iteration, without truncation (for clean evaluation)')
...
if __name__ == '__main__':
    ...
    attack_model = Implicit(modSiren, classifier, inner_steps=args.mod_steps, inner_lr = args.inner_lr, voxels = args.dataset=='modelnet',device=args.device)
