...
class TMO(nn.Module):
    def __init__(self, inr, classifier, tmo_steps=5, inner_steps = 100, inner_lr = 0.01, voxels = False, device='cuda'):
        """
        :param inr: pretrained inr model
        :param classifier: 'clean' pretrained classifier model to attack
        :param tmo_steps: Number of steps when performing truncated modulation optimization
    ...
    def fit_image(self, x, start_mod, clean, return_mse = False):
        ...
        for image in x:
            ...
            mse = 0
            for step in range(self.inner_steps if clean else self.tmo_steps):
...
def run_attack(model, loader, criterion, constraint, pgd_iters, pgd_lr, voxels = False, device='cuda'):
    ...
    for x,labels in prog_bar:
        ...
        for iter in range(pgd_iters):
            ...
            if voxels:
                output, cur_mod = model(mask(x).unsqueeze(1), clean_mod, clean=False)
                loss = 1-criterion(output,labels)
                loss.backward()
                mask.mask_grad(lr=pgd_lr) #We perform manual gradient step in the 3D case
            else:
                optimizer.zero_grad()
...
def get_args():
    ...
    parser.add_argument('--tmo-steps', type=int, default=5, help='Number of modulation optimization steps in truncation.')
...
if __name__ == '__main__':
    ...
    attack_model = TMO(modSiren, classifier, tmo_steps=args.tmo_steps, inner_steps=args.mod_steps, inner_lr = args.inner_lr, voxels = args.dataset=='modelnet',device=args.device)
