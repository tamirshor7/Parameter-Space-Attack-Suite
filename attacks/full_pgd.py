...
def run_attack(model, loader, criterion, constraint, pgd_iters, pgd_lr, device='cuda'):
    ...
    for x,labels in prog_bar:
        ...
        output, cur_mod, final_mse = model((x+proj_pert).detach().unsqueeze(1), clean_mod.detach(), clean=True, return_mse = True)
